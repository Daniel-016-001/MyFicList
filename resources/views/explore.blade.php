...
<head>
    ...
    <style>
        ...
        .card-hover:hover { transform: translateY(-8px); border-color: rgba(96, 165, 250, 0.5); box-shadow: 0 20px 40px -15px rgba(0, 0, 0, 0.5); }
    </style>
</head>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-12">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        <h1 class="text-4xl md:text-5xl font-black tracking-tighter bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 bg-clip-text text-transparent">
                            Explorar Catálogo
                        </h1>
                        <p class="text-gray-400 mt-2 font-medium">Descubre todo el contenido guardado en nuestra base de datos.</p>
                    </div>
                    ...
                    <form action="{{ route('media.explore') }}" method="GET" class="flex flex-wrap items-center gap-3">
                        ...
                        <select name="type" class="bg-gray-900/50 border border-white/10 rounded-2xl py-3 px-4 text-sm focus:ring-2 focus:ring-blue-500/50 outline-none transition-all cursor-pointer">
                        ...
                        <select name="genre" class="bg-gray-900/50 border border-white/10 rounded-2xl py-3 px-4 text-sm focus:ring-2 focus:ring-blue-500/50 outline-none transition-all cursor-pointer">
                        ...
                        <button type="submit" class="bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 px-6 rounded-2xl shadow-lg shadow-blue-600/20 transition-all">
                            Filtrar
                        </button>

                        @if(request()->anyFilled(['search', 'type', 'genre']))
                            <a href="{{ route('media.explore') }}" class="text-gray-500 hover:text-white transition-colors text-sm font-bold ml-2">
                                Limpiar
                            </a>
                        @endif
                    </form>
            ...
            <div id="media-grid" class="columns-2 sm:columns-3 md:columns-4 lg:columns-5 xl:columns-6 gap-6">
                @forelse($mediaItems as $media)
                    @php
                        ...
                        foreach($platforms as $p) {
                            $pLower = strtolower($p);
                            if (str_contains($pLower, 'pc') || str_contains($pLower, 'windows')) $platformIcons[] = 'fab fa-windows';
                            if (str_contains($pLower, 'playstation') || str_contains($pLower, 'ps')) $platformIcons[] = 'fab fa-playstation';
                            if (str_contains($pLower, 'xbox')) $platformIcons[] = 'fab fa-xbox';
                            if (str_contains($pLower, 'nintendo') || str_contains($pLower, 'switch')) $platformIcons[] = 'fab fa-nintendo-switch';
                        }
                        ...
                        <div class="bg-slate-900 rounded-2xl overflow-hidden shadow-2xl transition-transform duration-300 hover:scale-[1.02] border border-blue-900/20 flex flex-col">
                            ...
                            <a href="{{ route('media.show', $media->id) }}" class="block relative group cursor-pointer">
                                ...
                                @if(!empty($extra['trailer_url']))
                                    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-black/40 backdrop-blur-md flex items-center justify-center border border-white/10 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <div class="p-5 flex-grow flex flex-col">
                                ...
                                <h3 class="font-bold text-lg text-white leading-tight mb-4">
                                    {{ $media->title }}
                                </h3>
                                ...
                                <div class="mt-auto flex items-center justify-between pt-4 border-t border-slate-800/50">
                                    @auth
                                        <button onclick="openListModal({{ $media->id }})" class="text-blue-400 hover:text-blue-300 flex items-center gap-1.5 text-xs font-bold transition-colors">
                                            <i class="fas fa-plus-circle"></i> Agregar
                                        </button>
                                    @else
                                        <a href="{{ route('login') }}" class="text-blue-400 hover:text-blue-300 flex items-center gap-1.5 text-xs font-bold transition-colors">
                                            <i class="fas fa-plus-circle"></i> Agregar
                                        </a>
                                    @endauth
                                    <a href="{{ route('media.show', $media->id) }}" class="text-gray-400 hover:text-white flex items-center gap-1.5 text-xs font-bold transition-colors">
                ...
                @empty
                    <div class="col-span-full py-24 text-center">
                        <div class="w-20 h-20 bg-slate-900 rounded-3xl flex items-center justify-center mx-auto mb-6 border border-blue-900/20">
                        ...
                        <a href="{{ route('media.explore') }}" class="mt-6 inline-block bg-blue-600 text-white font-bold px-8 py-3 rounded-2xl hover:bg-blue-500 transition-colors">
