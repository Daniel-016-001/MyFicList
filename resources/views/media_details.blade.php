        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1">
                <div class="sticky top-8">
                    <img src="{{ data_get($details, 'cover_url') }}" alt="{{ data_get($details, 'title') }}" class="w-full rounded-lg shadow-lg mb-6">
            ...
            <div class="lg:col-span-2">
                <div class="bg-gray-900 rounded-lg p-6 border border-gray-800">
                    <div class="mb-6">
                        <span class="text-xs font-bold uppercase text-blue-400 bg-blue-900/30 px-2 py-1 rounded mb-2 inline-block">{{ data_get($details, 'source') }}</span>
                        <h1 class="text-3xl font-bold text-gray-100 mb-2">{{ data_get($details, 'title') }}</h1>
                        @if(data_get($details, 'original_title') && data_get($details, 'original_title') !== data_get($details, 'title'))
                            <p class="text-gray-400 text-lg">{{ data_get($details, 'original_title') }}</p>
                        @endif
                    </div>
                        ...
                        <div class="mb-6">
                            ...
                            @if($youtubeId)
                                <div class="aspect-video rounded-xl overflow-hidden border border-gray-700">
                                    <iframe class="w-full h-full" src="https://www.youtube.com/embed/{{ $youtubeId }}" title="Tráiler de {{ data_get($details, 'title') }}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                                </div>
                            @else
                                <a href="{{ data_get($details, 'trailer_url') }}" target="_blank" rel="noreferrer" class="inline-block bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-lg transition-colors">
                    ...
                    @if(!empty(data_get($details, 'images')) && count(data_get($details, 'images')) > 0)
                        <div class="mb-6">
                            <h3 class="text-lg font-semibold text-gray-100 mb-2">Imágenes</h3>
                            <div class="grid grid-cols-2 gap-3">
                                @foreach(array_slice(data_get($details, 'images'), 0, 4) as $image)
                                    <img src="{{ $image }}" alt="Imagen de {{ data_get($details, 'title') }}" class="w-full h-40 object-cover rounded-lg border border-gray-700">
                                @endforeach
                            </div>
                        </div>
                    @endif
                ...
                <div class="p-6">
                    ...
                    <form id="addForm" action="{{ route('user-list.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" id="modalProgress" name="progress" min="0" value="0" class="w-full bg-gray-800 border border-gray-600 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-blue-500" style="background-color:#1f2937;color:#f8fafc;" placeholder="0">
                        </div>
